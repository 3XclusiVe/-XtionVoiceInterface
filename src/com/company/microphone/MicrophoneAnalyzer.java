...
/********************************************************************************************
 * com.company.microphone.Microphone Analyzer class, detects pitch and volume while extending the microphone class.
...
public class MicrophoneAnalyzer extends Microphone {
    ...
    private double findMeanBetween(int minFreq, int maxFreq, Complex[] result) {
        ...
        int minIndex = (int) (((float)minFreq / (sampleRate / 2)) * result.length);
        //System.out.println("@@ " + result[68].getMagnitude());
        int maxIndex = (int) (((float)maxFreq / (sampleRate / 2)) * result.length);

        double magnitude = 0;

        for(int i = minIndex; i < maxIndex; i++ ) {
            magnitude += result[i].getMagnitude();
        }
